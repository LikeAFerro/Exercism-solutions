...
static void test_the_factor_0_does_not_affect_the_sum_of_multiples_of_other_factors(void) {
    const unsigned int multiples[] = {3, 0};
    TEST_ASSERT_EQUAL(3, sum(multiples, NUM_OF_ELEMENTS(multiples), 4));
}

static void test_solutions_using_include_exclude_must_extend_to_cardinality_greater_than_3(void) {
    const unsigned int multiples[] = {2, 3, 5, 7, 11};
    TEST_ASSERT_EQUAL(39614537, sum(multiples, NUM_OF_ELEMENTS(multiples), 10000));
}
